function submitDetails(event) {
  ...
  let partnerMbti = document.getElementById("partnerMbtiInput").value;
  ...
  // MBTI 형식 유효성 검사 (대소문자 모두 허용)
  const mbtiPattern = /^[A-Za-z]{4}$/;
  if (!mbtiPattern.test(myMbti) || !mbtiPattern.test(partnerMbti)) {
    alert("정확한 MBTI 형식을 입력해 주세요. 예: ENFP");
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".relationship-btn").forEach((button) => {
    button.addEventListener("click", function () {
      document
        .querySelectorAll(".relationship-btn")
        .forEach((btn) => btn.classList.remove("selected"));
      this.classList.add("selected");
      document.getElementById("selectedRelationship").value = this.value;
    });
  });
